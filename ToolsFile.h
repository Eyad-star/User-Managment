...
namespace ToolFile
{
    void AddOneLineToFile(const string& Line)
    {
        fstream MyFile;
        MyFile.open("D:\\ProjectUsers\\DateUsers.txt", ios::app);
        if(MyFile.is_open())
        {
            MyFile<<Line<<"\n";
        }
    ...
    void SaveOneAccountToFile(const string& Account)
    {
        AddOneLineToFile(Account);
    }
